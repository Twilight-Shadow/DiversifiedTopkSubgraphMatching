...
def make_data_source(args):
    ...
    #         raise Exception("Error: unrecognized dataset")
    # add_start
    data_source = data.DiskDataSource_c(node_anchored=args.node_anchored)
    # add_end
    return data_source
    ...
    while not done:
        ...
        for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
            ...
            pos_a, pos_b, neg_a, neg_b = data_source.gen_batch(batch_target,
                batch_neg_target, batch_neg_query, True, p_batch)
...
def train_loop(args):
    ...
    p_batch = 0
    for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
        # print(batch_neg_query, batch_neg_target)
        pos_a, pos_b, neg_a, neg_b, ground_truth = data_source.gen_batch(batch_target,
            batch_neg_target, batch_neg_query, False, p_batch)
        # pos_a, pos_b, neg_a, neg_b = data_source.gen_batch(batch_target,
        ...
        test_pts.append((pos_a, pos_b, neg_a, neg_b, ground_truth))
        p_batch += 1

    # print(test_pts[0][3])
    # print(type(test_pts[0][3]))
