...
class OTFSynDataSource(DataSource):
    ...
    """
    def __init__(self, max_size=100, min_size=5, n_workers=4,
...
class DiskDataSource(DataSource):
    ...
    def gen_batch(self, a, b, c, train, max_size=15, min_size=5, seed=None,
        ...
        print("?")
        print(list(pos_a[0].nodes(data=True)))
        print(list(pos_b[0].nodes(data=True)))
        print(pos_a_anchors[0])
        print(pos_b_anchors[0])
...
class DiskDataSource_c(DataSource):
    def __init__(self, node_anchored=False):
        self.node_anchored = node_anchored
        # self.dataset = load_dataset(dataset_name)
        # self.min_size = min_size
        # self.max_size = max_size
    ...
    def gen_batch(self, a, b, c, train, p_batch, max_size=10, min_size=5, seed=None,
        filter_negs=True, sample_method="tree-pair"):
        batch_size = a
        # train_set, test_set, task = self.dataset
        # graphs = train_set if train else test_set
        if seed is not None:
        ...
        # with open("./dataset_dblp/ground_truth_1.txt") as file:
        #     for line in file:
        #         split_line = line.strip().split()
        #         for truth in split_line:
        #             ground_truth.append(int(truth))

        # print(p_batch)

        # ground_truth = ground_truth[p_batch * 64: (p_batch + 1) * 64]

        # print(ground_truth)

        for i in range(p_batch * 512, (p_batch + 1) * 512):
            
            # print(ground_truth[i - p_batch * 64])
            pre_index = random.randint(0, 999)
            with open("./dataset_dblp/dataset/" + str(pre_index) + ".txt") as file: 
                node_set = set()
                node_lable = {}
                edge = []
                for line in file:
                    split_line = line.strip().split()
                    node_set.add(int(split_line[0]))
                    node_set.add(int(split_line[1]))
                    node_lable[int(split_line[0])] = torch.tensor([int(split_line[2])], dtype=torch.float)
                    node_lable[int(split_line[1])] = torch.tensor([int(split_line[3])], dtype=torch.float)
                    # node_lable[int(split_line[0])] = torch.tensor([random.randint(0, 1)], dtype=torch.float)
                    # node_lable[int(split_line[1])] = torch.tensor([random.randint(0, 1)], dtype=torch.float)
                    edge.append((int(split_line[0]), int(split_line[1])))
                ...
                node_set.sort()
                for j in node_set:
                    G_a.add_node(j, node_feature=node_lable[j])
                # for j in node_set:
                #     G_a.add_node(j)
                G_a.add_edges_from(edge)
                _GA, node_set = utils.sample_neigh([G_a], random.randint(6, 29))
                G_a = G_a.subgraph(node_set)
                choose = random.randint(0, 1)
                # print(choose)
                if choose == 0:
                    # print(i)
                    G_b = nx.Graph()
                    # edge_b = edge[:random.randint(min_size, max_size)]
                    # node_set_b = set()
                    # for j in edge_b:
                    #     node_set_b.add(j[0])
                    #     node_set_b.add(j[1])
                    # node_set_b = list(node_set_b)
                    # node_set_b.sort()
                    # for j in node_set_b:
                    #     G_b.add_node(j, node_feature=node_lable[j])
                    # # for j in node_set_b:
                    # #     G_b.add_node(j)
                    # G_b.add_edges_from(edge_b)
                    G_b, node_set_b = utils.sample_neigh([G_a], random.randint(3, len(node_set)))
                    G_b = G_a.subgraph(node_set_b)
                    pos_a.append(G_a)
                    pos_b.append(G_b)
                    pos_a_anchors.append(list(node_set_b)[0])
                    pos_b_anchors.append(list(node_set_b)[0])
                else:
                    # print(i)
                    fin = False
                    while fin != True:
                        ...
                        while index != pre_index:
                            index = random.randint(0, 999)
                        with open("./dataset_dblp/dataset/" + str(index) + ".txt") as _file: 
                            _node_set = set()
                            _node_lable = {}
                            _edge = []
                            for line in _file:
                                split_line = line.strip().split()
                                _node_set.add(int(split_line[0]))
                                _node_set.add(int(split_line[1]))
                                _node_lable[int(split_line[0])] = torch.tensor([int(split_line[2])], dtype=torch.float)
                                _node_lable[int(split_line[1])] = torch.tensor([int(split_line[3])], dtype=torch.float)
                                # _node_lable[int(split_line[0])] = torch.tensor([random.randint(0, 1)], dtype=torch.float)
                                # _node_lable[int(split_line[1])] = torch.tensor([random.randint(0, 1)], dtype=torch.float)
                                _edge.append((int(split_line[0]), int(split_line[1])))
                        # _edge_b = _edge[:random.randint(min_size, max_size)]
                        _node_set_b = set()
                        for j in _edge:
                            _node_set_b.add(j[0])
                            _node_set_b.add(j[1])
                        _node_set_b = list(_node_set_b)
                        _node_set_b.sort()
                        for j in _node_set_b:
                            G_b.add_node(j, node_feature=_node_lable[j])
                        # for j in _node_set_b:
                            # G_b.add_node(j)
                        G_b.add_edges_from(_edge)
                        _G_b, _node_set_b = utils.sample_neigh([G_b], random.randint(3, len(node_set)))
                        _G_b = G_b.subgraph(_node_set_b)
                        matcher = nx.algorithms.isomorphism.GraphMatcher(G_a, _G_b)
                        if matcher.subgraph_is_isomorphic(): # a <= b (b is subgraph of a)
                            continue
                        neg_a.append(G_a)
                        neg_b.append(_G_b)
                        neg_a_anchors.append(list(node_set)[0])
                        neg_b_anchors.append(list(_node_set_b)[0])
                        fin = True

        # print(list(neg_a[0].nodes(data=True)))
        # print(list(neg_b[0].nodes(data=True)))
        # print(neg_a_anchors[0])
        # print(neg_b_anchors[0])
        # print(p_batch, len(pos_a), len(pos_b), len(neg_a), len(neg_b))
        # print(list(pos_a[0].edges))
        # print(list(pos_b[0].edges))
        # print(list(neg_b[0].edges))
        # output_cnt = 0
        # for i in pos_b:
        #     with open("./dataset_dblp/rquery/batch" + str(p_batch) + "pos" + str(output_cnt) + ".txt", 'w') as ofile:
        #         ofile.write(str(i.nodes()) + '\n')
        #         ofile.write(str(i.edges()) + '\n')
        #     output_cnt += 1
        # output_cnt = 0
        # for i in neg_b:
        #     with open("./dataset_dblp/rquery/batch" + str(p_batch) + "neg" + str(output_cnt) + ".txt", 'w') as ofile:
        #         ofile.write(str(i.nodes()) + '\n')
        #         ofile.write(str(i.edges()) + '\n')
        #     output_cnt += 1
