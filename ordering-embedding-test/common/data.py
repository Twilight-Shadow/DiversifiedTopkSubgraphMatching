...
class DiskDataSource_c(DataSource):
    ...
    def gen_batch(self, a, b, c, train, p_batch, max_size=15, min_size=5, seed=None,
        filter_negs=False, sample_method="tree-pair"):
        batch_size = a
        # train_set, test_set, task = self.dataset
        # graphs = train_set if train else test_set
        ...
        if p_batch == 0:
            with open("./dataset_wordnet18/ground_truth.txt") as file:
                for line in file:
                    split_line = line.strip().split()
                    for truth in split_line:
                        ground_truth.append(int(truth))

        # ground_truth = ground_truth[p_batch * 64: min((p_batch + 1) * 64, 1000)]64
        ...
        anchor = list()
        with open("./dataset_wordnet18/anchor_info/3.txt", "r") as file:
            for line in file:
                split_line = line.strip().split()
                anchor.append([int(split_line[0]), int(split_line[1])])

        for i in range(p_batch * 78, min((p_batch + 1) * 78, 78)):
            # print(ground_truth[i - p_batch * 64])
            with open("./dataset_wordnet18/dataset/" + str(i) + ".txt") as file: 
                node_set = set()
                node_lable = {}
                edge = []
                for line in file:
                ...
                path_lengths = nx.single_source_shortest_path_length(G_a, anchor[i][1] if anchor[i][1] != -1 else list(node_set)[0])
                k_hop_nodes = [node for node, length in path_lengths.items() if length <= 3]
                G_a = G_a.subgraph(k_hop_nodes)
                if anchor[i][1] != -1:
                    neg_a_anchors.append(anchor[i][1])
                    neg_b_anchors.append(anchor[i][1])
                else:
                    neg_a_anchors.append(list(node_set)[0])
                    neg_b_anchors.append(list(node_set)[0])
                pos_a.append(G_a)
                neg_a.append(G_a)

            with open("./dataset_wordnet18/query/" + str(3) + ".txt") as file:
                node_set = set()
                node_lable = {}
                edge = []
                for line in file:
                ...
                G_b.add_edges_from(edge)
                if anchor[i][0] != -1:
                    pos_a_anchors.append(anchor[i][0])
                    pos_b_anchors.append(anchor[i][0])
                else:
                    pos_a_anchors.append(list(node_set)[0])
                    pos_b_anchors.append(list(node_set)[0])
                pos_b.append(G_b)
                neg_b.append(G_b)
                # print(node_set)

        # print(p_batch, len(pos_a), len(pos_b), len(neg_a), len(neg_b))
        # print(neg_a[0].nodes(data=True))
        # print(pos_b[0].nodes(data=True))
        # print(neg_a[0].edges())
        # print(pos_a[0].edges())
        ...
        neg_b = utils.batch_nx_graphs(neg_b, anchors=neg_b_anchors if
            self.node_anchored else None)
        return pos_a, pos_b, neg_a, neg_b, ground_truth
