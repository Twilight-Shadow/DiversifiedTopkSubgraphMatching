...
enum OptionKeyword {
    ...
    k = 15,                             // -k, selected partition number
    WholeGraphPath = 16,                // -w, the whole graph
    PartitionNum = 17,                  // -p, partition number
    QueryNum = 18                       // -qry, query number
};
...
public:
    ...
    std::string getk() {
        return options_value[OptionKeyword::k] == "" ? "4" : options_value[OptionKeyword::k];
    }

    std::string getWholeGraphPath() {
        return options_value[OptionKeyword::WholeGraphPath]  == "" ? "ERR" : options_value[OptionKeyword::WholeGraphPath];
    }

    std::string getPartitionNum() {
        return options_value[OptionKeyword::PartitionNum] == "" ? "ERR" : options_value[OptionKeyword::PartitionNum];
    }

    std::string getQueryNum() {
        return options_value[OptionKeyword::QueryNum] == "" ? "ERR" : options_value[OptionKeyword::QueryNum];
    }
};
