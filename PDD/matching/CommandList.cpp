...
MatchingCommand::MatchingCommand(const int argc, char **argv) : CommandParser(argc, argv) {
    ...
    options_key[OptionKeyword::WholeGraphPath] = "-w";
    options_key[OptionKeyword::PartitionNum] = "-p";
    options_key[OptionKeyword::QueryNum] = "-qry";
    processOptions();
};
...
void MatchingCommand::processOptions() {
    ...
    // Whole Graph Path
    options_value[OptionKeyword::WholeGraphPath] = getCommandOption(options_key[OptionKeyword::WholeGraphPath]);

    // Partition number
    options_value[OptionKeyword::PartitionNum] = getCommandOption(options_key[OptionKeyword::PartitionNum]);

    // Query number
    options_value[OptionKeyword::QueryNum] = getCommandOption(options_key[OptionKeyword::QueryNum]);
}